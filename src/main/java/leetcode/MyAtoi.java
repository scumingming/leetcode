package com.mm.learn.leetcode;

import java.math.BigInteger;

/**
 * Created by wangmingming on 2019/4/15.
 * 请你来实现一个 atoi 函数，使其能将字符串转换成整数。

 首先，该函数会根据需要丢弃无用的开头空格字符，直到寻找到第一个非空格的字符为止。

 当我们寻找到的第一个非空字符为正或者负号时，则将该符号与之后面尽可能多的连续数字组合起来，
 作为该整数的正负号；假如第一个非空字符是数字，则直接将其与之后连续的数字字符组合起来，形成整数。
 该字符串除了有效的整数部分之后也可能会存在多余的字符，这些字符可以被忽略，它们对于函数不应该造成影响。

 注意：假如该字符串中的第一个非空格字符不是一个有效整数字符、字符串为空或字符串仅包含空白字符时，则你的函数不需要进行转换。

 在任何情况下，若函数不能进行有效的转换时，请返回 0。

 说明：

 假设我们的环境只能存储 32 位大小的有符号整数，那么其数值范围为 [−231,  231 − 1]。如果数值超过这个范围，
 qing返回  INT_MAX (231 − 1) 或 INT_MIN (−231) 。
 java.lang.NumberFormatException: For input string: "20000000000000000000"
 at line 65, java.lang.NumberFormatException.forInputString
 at line 592, java.lang.Long.parseLong
 at line 803, java.lang.Long.valueOf
 at line 25, Solution.myAtoi
 at line 54, __DriverSolution__.__helper__
 at line 79, __Driver__.main
 */
public class MyAtoi {

    public static void main(String[] args) {

        System.out.println(new MyAtoi().myAtoi("-0667"));
    }

    public int myAtoi(String str) {
        double ans = 0;

        String newStr = str.trim();
        boolean isNegtive = false;
        if (newStr.startsWith("+")) {
            newStr = newStr.substring(1);
        } else if(newStr.startsWith("-")) {
            newStr = newStr.substring(1);
            isNegtive = true;
        }
        int i = 0;
        while (i <= newStr.length() - 1)           {
            if('0' == newStr.charAt(i)) {
                i++;
            } else {
                break;
            }
        }
        System.out.println(i);

        while (i <= newStr.length() - 1) {
            if(newStr.charAt(i) >= '0' && newStr.charAt(i) <= '9'){
                ans = ans*10 + Integer.valueOf(newStr.substring(i,i+1));
                i++;
            } else {
                break;
            }
        }
        ans = isNegtive ? 0 - ans : ans;
        if (ans > Integer.MAX_VALUE) return Integer.MAX_VALUE;
        if (ans < Integer.MIN_VALUE) return Integer.MIN_VALUE;
        return (int)ans;
    }
}
